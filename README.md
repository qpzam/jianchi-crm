...
### 🎯 比同行快 2 小时
传统方式需要人工刷巨潮网、逐份阅读PDF、手动搜索联系方式，一套流程下来至少2小时。本系统从公告发布到生成带联系方式的日报，全程自动化，仅需5分钟。

### 🤖 GPT 全量语义解析，不靠正则
不依赖脆弱的正则表达式提取数据。每份PDF都由AI完整理解后提取股东名称、减持比例、窗口期等关键信息，准确率>95%，支持多股东、复杂表格等传统工具无法处理的场景。

### 🧠 AI 智能备注，辅助决策
系统自动为每条公告生成智能备注：识别高比例减持、机构股东、窗口临近等关键信号，帮助业务人员快速判断优先级，把精力聚焦在最有价值的线索上。

### ⏰ 每天早8点，打开电脑就能用
crontab + pmset 双重保障，Mac合盖休眠即可。每个工作日早上自动唤醒、抓取、解析、匹配、生成报告，打开电脑直接看到今日减持清单。

### 🔒 数据安全，本地运行
所有数据存储在本地SQLite数据库，联系方式库不上传云端，不依赖第三方SaaS。你的客户数据，只在你的电脑上。
